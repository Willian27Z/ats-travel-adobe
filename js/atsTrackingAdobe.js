...
if(/details\.html/.test(window.location.pathname)){
	...
	*/
	digitalData.pageCategory = "product detail";
	digitalData.products.push(travelDestination);
...
if(/checkout\.html/.test(window.location.pathname)){
	...
	if(JSON.parse(getCookie('55Basket')) || cart55.length <1){
		...
		*/
		digitalData.pageCategory = "checkout";
...
if(/thankyou\.html/.test(window.location.pathname)){
	...
	*/
	digitalData.pageCategory = "purchase confirmation";
	digitalData.transaction = order55[order55.length -1].orderRef;
	digitalData.totalPrice = totalPrice;
	digitalData.products = products;
